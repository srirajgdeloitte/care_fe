    ...
    if (inventory && inventory.length) {
        inventoryList = inventory.map((inventoryItem: any) => (
            <tr key={inventoryItem.id} className="bg-white" >
                ...
                <td className="px-5 py-5 border-b border-gray-200 text-sm ">
                    <Button className="ml-2 bg-blue-400 hover:bg-blue-600" onClick={() => navigate(`/facility/${facilityId}/inventory/${inventoryItem.id}/update/${inventoryItem.item_object?.id}`)}>
                        UPDATE
                    </Button>
                </td>
            </tr>
