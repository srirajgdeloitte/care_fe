...
import { getMBBSStudents } from "../../Redux/actions";

export const MBBSStudentSelect = (props: any) => {
  ...
  const fetchUsers = useCallback(
    async (status: statusType) => {
      ...
      const res = await dispatchAction(getMBBSStudents());
      if (!status.aborted) {
        if (res && res.data) {
  ...
  const selectedMBBSStudent = state.users.find((item: any) => item.id == userId);
  return (
    <div className="pb-2">
      <div className="space-y-1">
        <label id="listbox-label" className="block text-sm leading-5 font-medium text-gray-700">
          Assigned to MBBS student
        </label>
                ...
                <div className="space-x-3 flex items-center">
                  <span aria-label="Online"
                    className={"flex-shrink-0 inline-block h-2 w-2 rounded-full " +
                      (selectedMBBSStudent ? (moment().subtract(5, 'minutes').isBefore(selectedMBBSStudent.last_login) ? "bg-green-400" : "bg-gray-300") : "bg-blue-400")
                    }
                  >
                  </span>
                  <span className="block truncate">
                    {selectedMBBSStudent ? selectedMBBSStudent.first_name + ' ' + selectedMBBSStudent.last_name : "Assign a MBBS student"}
                  </span>
                </div>
